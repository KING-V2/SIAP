...
export class ListaTipoDocumentoComponent implements OnInit {
  ...
  constructor(private tipodocumentoService: TipoDocumentoService, private alertasService: AlertasService, private authService:AuthService) { }
  ...
  obtenerTipoDocumento() {
    ...
    this.tipodocumentoService.obtenerTipoDocumento().subscribe(
      data => {
        console.log("Datos de tipodocumentos obtenidos:", data);
        this.tipodocumentos = of(data);
      },
      error => {
        console.error("Error al obtener las tipodocumentos:", error);
        if (error.status === 403) {
