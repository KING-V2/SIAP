...
export class ListaDevolucionComponent implements OnInit {
  ...
  constructor(private devolucionService: DevolucionService, private alertasService: AlertasService, private authService: AuthService) { }
  ...
  obtenerDevolucion() {
    ...
    this.devolucionService.obtenerDevolucion().subscribe(
      data => {
        console.log("Datos de devluciones obtenidos:", data);
        this.devoluciones = of(data);
      },
      error => {
        console.error("Error al obtener las devluciones:", error);
        if (error.status === 403) {
  ...
  borrarDevolucion(id: string) {
    ...
    if (userRole === 1) {
      console.log("ID de la devolucion a eliminar:", id);
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.devolucionService.borrarDevolucion(id).subscribe(
            () => {
              Swal.fire({
                title: "¡Eliminado!",
